...
extension NetworkService: NetworkServiceSpec {}

public let service = NetworkService.default
...
public class NetworkService {
    ...
    /// ⚠️ Note. capture service until receive response, or use `service` singleton.
    public init(
        session: NetworkSession = DefaultNetworkSession()
    ) {
...
public extension NetworkService {
    ...
    func send<R: NetworkRequest>(
        ...
        return send(request, decision: nil, completion: completion)
    }

    @discardableResult
    private func send<R: NetworkRequest>(
        _ request: R,
